...
Brokerages are defined in [the following folder folder](/src/BrokerageExportProviders/Brokerages/). Each brokerage should be self-contained
in a folder, and not leak any of its details to the rest of the system.
...
- getBrokerEventsForReport
  - Given one of the files found through the above method, convert the said file into an instance of a `CommonBrokerageEvents` compatible
    type
- mergeBrokerEvents
...
Define a schema for the entire report the broker generates. This will give you autocomplete in IDEs, while also making the code less bug
prone.

### Create separate Extract and Transform scripts

Separate Extracting (of `CommonBrokerageEvents`) and their Transforming (to `StagingFinancialGrouping`) into their own files. This will make
it easier to tell what's happening where.
...
Makes tests for the broker and test out implementation of the `CommonBrokerageExportProvider` abstract class. At minimum, write tests
specifically for the Extract and Transform logics of the broker.
