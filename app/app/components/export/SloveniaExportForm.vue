...
async function onSubmit(valid: boolean) {
	...
	try {
		...
		emit("generated", { xml, csv, xmlFilename: filenames.xml, csvFilename: filenames.csv, authorityId: "slovenia", reportTypeId: reportType.value });
	} catch (e) {
	...
	<AppForm class="flex flex-col gap-4" @submit="onSubmit">
		<div class="card card-padding-md flex flex-col gap-3">
			...
			<div class="grid grid-cols-2 gap-3">
				<AppNumberInput v-model="year" :label="t('year_label')" :min="2010" :max="2100"
					:validate="validateYear" />
				<AppSelect v-model="reportType" :label="t('report_type_label')"
					:placeholder="t('report_type_placeholder')" :options="reportTypeOptions"
