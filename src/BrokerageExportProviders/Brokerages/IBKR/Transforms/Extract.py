...
def extractCorporateActions(node: etree.ElementBase) -> s.CorporateAction:
    notesAndCodesParsed = parseNotes(node.attrib.get("code", ""))

    att = node.attrib
    corporateAction = s.CorporateAction(
        ClientAccountID=att["accountId"],
        AccountAlias=att["acctAlias"],
        Currency=att["currency"],
        Model=s.Model(att["model"]) if att.get("model", "") != "" else None,
        FXRateToBase=float(att["fxRateToBase"]),
        AssetClass=s.AssetClass(att["assetCategory"]),
        SubCategory=s.SubCategory(att["subCategory"]),
        Symbol=att["symbol"],
        Description=att["description"],
        Conid=att["conid"],
        SecurityID=att["securityID"],
        SecurityIDType=s.SecurityIDType(att["securityIDType"]),
        CUSIP=utils.valueOrNone(att.get("cusip", "")),
        ISIN=att["isin"],
        FIGI=utils.valueOrNone(att.get("figi", "")),
        ListingExchange=att["listingExchange"],
        UnderlyingConid=utils.valueOrNone(att.get("underlyingConid", "")),
        UnderlyingSymbol=utils.valueOrNone(att.get("underlyingSymbol", "")),
        UnderlyingSecurityID=utils.valueOrNone(att.get("underlyingSecurityID", "")),
        UnderlyingListingExchange=utils.valueOrNone(att.get("underlyingListingExchange", "")),
        Multiplier=float(att["multiplier"]),
        Strike=utils.floatValueOrNone(att.get("strike", "")),
        Expiry=(utils.safeDateParse(att["expiry"]) if att.get("expiry", "") != "" else None),
        PutOrCall=(s.PutOrCall(utils.valueOrNone(att["putCall"])) if utils.valueOrNone(att.get("putCall", "")) is not None else None),
        PrincipalAdjustFactor=utils.floatValueOrNone(att.get("principalAdjustFactor", "")),
        ReportDate=utils.safeDateParse(att["reportDate"]),
        DateTime=utils.safeDateParse(att["dateTime"]),
        ActionDescription=att["actionDescription"],
        Amount=float(att["amount"]),
        Proceeds=float(att["proceeds"]),
        Value=float(att["value"]),
        Quantity=float(att["quantity"]),
        FifoProfitAndLossRealized=float(att["fifoPnlRealized"]),
        CapitalGainsProfitAndLoss=float(att.get("capitalGainsPnl", "0")),
        ForexProfitAndLoss=float(att.get("fxPnl", "0")),
        MarketToMarketProfitAndLoss=utils.floatValueOrNone(att.get("mtmPnl", "")),
        NotesAndCodes=notesAndCodesParsed,
        Type=att["type"],
        TransactionID=att["transactionID"],
        ActionID=att["actionID"],
        LevelOfDetail=s.LevelOfDetail(att["levelOfDetail"]),
        SerialNumber=utils.valueOrNone(att.get("serialNumber", "")),
        DeliveryType=utils.valueOrNone(att.get("deliveryType", "")),
        CommodityType=utils.valueOrNone(att.get("commodityType", "")),
        Fineness=float(att.get("fineness", "0.0")),
        Weight=float(att.get("weight", "0.0")),
    )
...
def extractStockTrade(node: etree.ElementBase) -> s.TradeStock:
    ...
    trade = s.TradeStock(
        ...
        NetCashInBase=utils.floatValueOrNone(node.get("netCashInBase", "")),
        ...
        CapitalGainsProfitAndLoss=utils.floatValueOrNone(node.get("capitalGainsPnl", "")),
        ForexProfitAndLoss=utils.floatValueOrNone(node.get("fxPnl", "")),
        MarketToMarketProfitAndLoss=utils.floatValueOrNone(node.get("mtmPnl", "")),
