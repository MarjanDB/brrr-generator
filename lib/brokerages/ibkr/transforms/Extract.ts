...
import { AssetClass, type BuyOrSell, type CashTransactionType, type Codes, type LevelOfDetail, type Model, type OpenCloseIndicator, type OrderType, type PutOrCall, type SecurityIDType, type SubCategory, type TransactionType } from "@brrr/brokerages/ibkr/schemas/IbkrSchemas.ts";
...
function extractFromStatement(statement: Record<string, unknown>): SegmentedTrades {
  ...
  const cashTransactionNodes: Record<string, string>[] = toArray(cashNode?.CashTransaction as Record<string, string> | Record<string, string>[]);
  ...
  const corporateActionNodes: Record<string, string>[] = toArray(caNode?.CorporateAction as Record<string, string> | Record<string, string>[]);
