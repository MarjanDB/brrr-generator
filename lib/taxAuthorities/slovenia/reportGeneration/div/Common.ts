...
import { EDavkiDividendType, type EDavkiDividendReportLine } from "@brrr/taxAuthorities/slovenia/schemas/Schemas.ts";
...
function fillInMissingCompanyInformationForDividendLineAndRoundAmounts(
  ...
  try {
    const responsibleCompany = companyLookupProvider.getCompanyInfo(firstLine.dividendPayerIdentificationNumber);
    ...
    const relevantCountry = countryLookupProvider.getCountry(responsibleCompany.location.country);
