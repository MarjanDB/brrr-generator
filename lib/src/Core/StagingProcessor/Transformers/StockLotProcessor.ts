...
export function processStockLot(
	...
	const allBuys = references.filter(
		(t): t is TradeEventStockAcquired => t instanceof TradeEventStockAcquired,
	);
	const allSells = references.filter(
		(t): t is TradeEventStockSold => !(t instanceof TradeEventStockAcquired),
	);
	...
	try {
		...
		console.error(
			`Failed processing stock lot (ID: ${input.id}, FinancialIdentifier: ${input.financialIdentifier}), found no match`,
		);
